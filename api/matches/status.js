...
export default async function handler(req, res) {
  ...
  try {
    ...
    const joinRows = await sql`
      SELECT mj.user_email, mj.joined_at, u.username, u.ff_uid
      FROM match_joins mj
      LEFT JOIN users u ON u.email = mj.user_email
    ...
    const players = joinRows.map((r) => ({
      email: r.user_email,
      username: r.username || r.user_email?.split("@")[0] || "Player",
      ffUid: r.ff_uid || "",
    ...
    if (session?.email) {
      myJoin = joinRows.find((r) => r.user_email === session.email) || null;
      if (myJoin) {
    ...
    const latestResult = resultRows[0]
      ? {
          ...
          winnerUsername: resultRows[0].winner_username,
          loserUsername: resultRows[0].loser_username,
          prizeCoins: Number(resultRows[0].prize_coins) || 15,
